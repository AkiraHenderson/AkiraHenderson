<!-- 1. 움직이는 헤더 배너 (twinkling 애니메이션 적용) -->
![header](https://capsule-render.vercel.app/api?type=waving&color=timeGradient&height=250&section=header&text=Welcome%20to%20H's%20Space&fontSize=70&animation=twinkling&fontAlignY=40)

### 🎮 About Me
* 🔭 **Project: THE SHADOWS** (내러티브 중심의 횡스크롤 액션 RPG) 프리 프로덕션 및 기획을 진행하고 있습니다.
* 🖋️ 게임 디자인 문서 작성, 시스템 플로우 차트 설계, 그리고 세계관 구축에 푹 빠져 있습니다.
* 🦉 주로 조용한 새벽 시간대에 커피와 함께 가장 많은 아이디어를 쏟아냅니다.
* 📐 게임 아키텍처와 로직 설계를 위해 탄탄한 수학적 기반을 다지는 중입니다.

<br/>

### 🛠️ Tech Stack & Tools
<!-- 본인이 사용하는 툴의 영문 이름을 쉼표로 연결하면 아이콘이 자동 생성됩니다 -->
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=figma,notion,markdown,github,discord,ps,ae" />
  </a>
</p>

<br/>

### 📊 GitHub Stats & Activity
<!-- 통계 카드 (다크/야간 테마 적용) -->
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_NAME&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_NAME&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117" width="48%" />
</p>

<!-- 잔디(커밋) 3D 애니메이션 위젯 -->
<br/>
<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_GITHUB_NAME&theme=tokyonight&hide_border=true&background=0D1117" />
</p>
